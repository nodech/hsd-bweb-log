...
class NameReporterOptions extends ReporterOptions {
  ...
  fromOptions(options) {
    ...
    this.Store = this.config.func('weblog-name-store', this.Store);

    this.fileName = this.config.str(
    ...
    this.fileMaxSize = this.config.mb(
      'weblog-name-file-size',
      this.fileMaxSize
    );
    this.fileMaxFiles = this.config.int(
      'weblog-name-max-files',
      this.fileMaxFiles
    );
